...
- **Task 11 设置中心与导入导出已实现**：`PrefItem`/`Pref`（`services/storage/Pref.ets`）按 AP-4 规范落地，
  导出仅含非默认项 + schemaVersion，导入只写回存在的键（缺失项保留原设置），重置调用 preferences clear；
  设置页（播放/外观/搜索/隐私/日志/关于）已接入"我的"页，导入导出走 `DocumentViewPicker` 文件选择/保存；
  深色模式即时生效、搜索历史开关联动搜索仓库、默认清晰度/倍速在视频详情生效。
